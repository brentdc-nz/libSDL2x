...
WatchGameController(SDL_GameController * gamecontroller)
{
    ...
    SDL_RenderClear(screen);
    //SDL_RenderPresent(screen); /* unresolved crash here */
    SDL_RaiseWindow(window);

    /* scale for platforms that don't give you the window size you asked for. */
    SDL_RenderSetLogicalSize(screen, SCREEN_WIDTH, SCREEN_HEIGHT);

	background = LoadTexture(screen, "D:\\controllermap.bmp", SDL_FALSE);
	button = LoadTexture(screen, "D:\\button.bmp", SDL_TRUE);
	axis = LoadTexture(screen, "D:\\axis.bmp", SDL_TRUE);
...
main(int argc, char *argv[])
{
	...
	/* Define the Xbox controller in a way that somewhat mimics an Xbox 360 controller */
	/* Triggers are digital in the SDL Xbox controller driver so they are defined as buttons */
